...
def uptade_produto():
    ...
    while True:
        ...
        try:
            ...
            match escolha:
                case 1:
                    produto_id = int(input('Digite o ID do produto que deseja trocar o nome:'))
                    novo_nome = input('Digite o novo nome do produto:')
                    sql = f"""UPDATE tb_produto SET nome = {novo_nome} where idt = {produto_id} """
                    cursor.execute(sql)
                    conexao.commit()
                case 2:
                    produto_id = int(input('Digite o ID do produto que deseja trocar o preço:'))
                    novo_preco = input('Digite o novo preço do produto:')
                    sql = f"""UPDATE tb_produto SET preco = {novo_preco} where idt = {produto_id}"""
                    cursor.execute(sql)
                    conexao.commit()
                case 3:
                    produto_id = int(input('Digite o ID do produto que deseja trocar a data de validade:'))
                    nova_data = input('Digite a nova data de validade do produto [aaaa-mm-dd]:')
                    sql = f"""UPDATE tb_produto SET dta_validade = {nova_data} where idt = {produto_id} """
                    cursor.execute(sql)
                    conexao.commit()
                case 4:
                    break
...
def update_cliente():
    ...
    while True:
        ...
        try:
            ...
            match escolha:
                case 1:
                    cliente_id = int(input('Digite o ID do cliente que deseja trocar o nome:'))
                    novo_nome = input('Digite o novo nome do cliente:')
                    sql = f"""UPDATE tb_cliente SET nome = {novo_nome} where idt = {cliente_id} """
                    cursor.execute(sql)
                    conexao.commit()
                case 2:
                    cliente_id = int(input('Digite o ID do cliente que deseja trocar a data de compra dele:'))
                    nova_data = input('Digite a data de compra que o cliente realizou [aaaa-mm-dd]:')
                    sql = f"""UPDATE tb_cliente SET dta_compra = {nova_data} where idt = {cliente_id}"""
                    cursor.execute(sql)
                    conexao.commit()
                case 3:
                    cliente_id = int(input('Digite o ID do cliente que deseja trocar o valor gasto pelo cliente:'))
                    novo_valor = input('Digite o valor gasto pelo cliente:')
                    sql = f"""UPDATE tb_cliente SET valor_gasto = {novo_valor} where idt = {cliente_id} """
                    cursor.execute(sql)
                    conexao.commit()
                case 4:
                    break
...
def delete_tb_produto():
    produto_id = int(input('Digite o ID do produto que deseja excluir a tupla:'))
    sql = f"""DELETE FROM tb_produto WHERE idt = {produto_id}"""
    cursor.execute(sql)


def delete_tb_cliente():
    cliente_id = int(input('Digite o ID do cliente que deseja excluir a tupla:'))
